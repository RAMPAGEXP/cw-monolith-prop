...
mod tests {
    ...
    const NATIVE_DENOM: &str = "urxp";

    #[test]
    fn initialization() {
        let mut deps = mock_dependencies();

        let withdraw_address = String::from("gordon-gekko-address"); // in reality this would be e.g. rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y
    ...
    fn sudo_send_all() {
        ...
        let funds_sent_to_contract = coins(5_000_000, NATIVE_DENOM);
        ...
        // in practice this test case should shake out
        // pretty much the same as the sudo send test case
        let msg = SudoMsg::ExecuteSendAll {
            recipient: community_nominated_address.clone(),
        };
        let res = sudo(deps.as_mut(), env, msg).unwrap();
        ...
        let msgs: Vec<CosmosMsg> = vec![BankMsg::Send {
            to_address: community_nominated_address,
            amount: coins(5_000_000, NATIVE_DENOM),
        }
        .into()];
