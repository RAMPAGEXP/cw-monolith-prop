...
if [ "$1" = "" ]
then
  echo "Usage: $0 1 arg required - rxp address"
  exit
fi
...
docker cp artifacts/cw_monolith_prop.wasm $CONTAINER_NAME:/cw_monolith_prop.wasm
...
# upload wasm
CONTRACT_CODE=$($BINARY tx wasm store "/cw_monolith_prop.wasm" --from validator $TXFLAG --output json | jq -r '.logs[0].events[-1].attributes[0].value')
...
# provision rxp default user i.e. rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y
...
$BINARY tx wasm instantiate $CONTRACT_CODE "$INIT" --from validator --label "rxp monolith prop" $TXFLAG --no-admin 
RES=$?
...
# send contract funds
$BINARY tx bank send rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y $CONTRACT_ADDRESS 2000000urxpx $TXFLAG

$BINARY tx gov submit-proposal sudo-contract $CONTRACT_ADDRESS '{"execute_send": {"amount": "1000000", "recipient": "rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y"}}' \
  --from test-user $TXFLAG \
  --title "Prop title" \
  --description "LFG" \
  --type sudo-contract \
  --deposit 500000000urxpx
